...
import argparse 
from algos import closest_to_cluster_centroids_approach_v4
import pandas as pd 
import time 
...
if __name__ == '__main__': 
    ...
    for angle in args.angles: 
        ...
        for mat_info in data_list[args.start:args.end]: 
            ...
            start = time.time()
            vol, selected_indices = closest_to_cluster_centroids_approach_v4(arr, num_entries=D, angle_threshold=angle,
        ...
        df.to_csv(EXP_PATH / 'OptimizationMatrices' / f'clustering_ang{int(angle)}_{args.num_mags}mag_v4.csv', 
                  index=False) 